...
"""
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
فاطمة الزهراء – الإصدار الأسطوري (Ollama + LLaMA3)
إحصائيات حية | هدايا نجوم حقيقية | قيود صارمة
■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
"""
...
HUNT_KEYWORDS = ["مشاركة", "انضمام", "سحب", "دخول", "روليت", "دب", "هدية", "نجوم", "تعزيز", "يلا", "سجل", "اضغط", "بسرعة", "التحق", "تأكيد", "شارك", "انقر"]
...
class FatimaBot:
    def __init__(self):
        ...
        self.cache = set()
        self.gift_log = []
        self.stats = {"wins":0, "stars":0, "gifts_bought":0, "gifts_converted":0, "channels_left":0, "start":time.time(), "msgs_processed":0}
        ...
        for _ in range(10):
            try:
                ...
                if await client.is_user_authorized():
                    logger.info(f"✅ {name}"); return True
    ...
    async def update_stats_msg(self, client):
        ...
        msg = (
            f"📊 **فاطمة الزهراء – إحصائيات حية**\n"
            f"🕒 التحديث: {datetime.now().strftime('%H:%M:%S')}\n"
            f"⏱️ مدة التشغيل: {uptime}\n"
            f"🏆 روليتات تم الصيد: {self.stats['wins']}\n"
            f"⭐ رصيد النجوم: ~{self.stats['stars']}\n"
            f"💎 هدايا مطورة مشتراة: {self.stats['gifts_bought']}\n"
            f"🎁 هدايا محولة: {self.stats['gifts_converted']}\n"
            f"🚪 قنوات غادرتها: {self.stats['channels_left']}\n"
            f"📨 رسائل معالجة: {self.stats['msgs_processed']}\n"
            f"🎯 القناص: {'🟢 مفعل' if self.sniper_enabled else '🔴 معطل'}"
        )
        try:
            if STATS_MSG_ID:
                await client.edit_message('me', STATS_MSG_ID, msg)
            else:
                sent = await client.send_message('me', msg)
                STATS_MSG_ID = sent.id
        except Exception as e:
            logger.warning(f"فشل تحديث الإحصائيات: {e}")

    # ---------- اكتشاف هدايا النجوم الحقيقية (رسائل الخدمة) ----------
    async def handle_star_gift(self, event, client):
        """عندما تأتي هدية نجوم حقيقية (MessageActionStarGift)، نحولها ونرد بشكر شرقي"""
        if isinstance(event.message.action, types.MessageActionStarGift):
            ...
            thanks = random.choice([
                "يسلمووو ❤️", "تسلم يالغالي 🌸", "واااي شكراً 💫",
                "يدك مبدوءة بالخير 💝", "الله يسعدك يا رب ✨"
            ])
    ...
    async def analyze_with_ai(self, text):
        try:
            system_prompt = (
                ...
                "تحللين الرسالة وتختارين الإجراء بصيغة JSON: "
    ...
    async def handle_command(self, event, parts, client):
        ...
        try:
            if cmd=="panel":
                ...
                await event.respond("🔥 **لوحة فاطمة الزهراء**", buttons=btns)
            elif cmd=="stats": await self.update_stats_msg(client); await event.reply("✅ تم تحديث الإحصائيات")
    ...
    async def main(self):
        ...
        await self.update_stats_msg(client)
        await client(UpdateStatusRequest(offline=False))
        logger.info("🚀 فاطمة الزهراء انطلقت")
